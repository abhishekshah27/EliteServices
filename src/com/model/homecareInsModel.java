...
public class homecareInsModel {
	...
	public List<HomecareBean> getData(){
		...
		try
		{
			...
			String sql="select * from homecare_master";
	...
	public int getID(String se_name)
	{
		...
		try
		{
			dbConnection db=new dbConnection();
			Connection conn=db.getConnection();
			String sql="select se_id from homecare_master where se_name=?";
			PreparedStatement pst=conn.prepareStatement(sql);
	...
	public void addService(homecareInsBean sp) {
