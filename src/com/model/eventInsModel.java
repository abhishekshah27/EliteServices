...
public class eventInsModel {
	...
	public List<EventBean> getData(){
		...
		try
		{
			...
			String sql="select * from events_master";
	...
	public int getID(String se_name)
	{
		...
		try
		{
			dbConnection db=new dbConnection();
			Connection conn=db.getConnection();
			String sql="select se_id from events_master where se_name=?";
			PreparedStatement pst=conn.prepareStatement(sql);
	...
	public void addService(eventInsBean sp) {
